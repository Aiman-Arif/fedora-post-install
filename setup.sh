...
func_proc () {
  ...
  local custom_texts=("${!1}")  # Indirect reference to array variable
  ...
  for ((i = 0; i < ${#custom_texts[@]}; i++)); do
    echo "$((i+1)). ${custom_texts[i]}"
  done
  ...
  if [ "$selected_indices" == "all" ]; then
    selected_indices=$(seq 1 ${#custom_texts[@]})
  fi
  ...
  for index in "${indices[@]}"; do
    if [[ $index =~ ^[0-9]+$ && $index -ge 1 && $index -le ${#custom_texts[@]} ]]; then
      echo -e "${BLUE}${custom_texts[index-1]} ${WHITE}"
...
func_proc custom_texts[@] custom_commands[@]